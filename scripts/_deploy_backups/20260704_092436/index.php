...
if ($path === '/admin/pages' && $METHOD === 'POST') {
    ...
    $visibility = in_array($BODY['visibility'] ?? '', VALID_VISIBILITY, true) ? $BODY['visibility'] : 'public';
    try {
        $id = run("INSERT INTO pages (title,slug,headline,store_slug,page_type,visibility) VALUES (?,?,?,?,?,?)",
            [$title, $slug, $BODY['headline']??null, $BODY['store_slug']??null, $pageType, $visibility]);
...
if (preg_match('#^/admin/pages/(\d+)$#', $path, $m)) {
    ...
    if ($METHOD === 'PUT') {
        ...
        try {
            run("UPDATE pages SET title=?,slug=?,headline=?,store_slug=?,is_active=?,theme=?,page_type=?,visibility=?,status=?,updated_at=datetime('now') WHERE id=?",
                [$BODY['title']??$page['title'], $slug, $BODY['headline']??$page['headline'],
                 $BODY['store_slug']??$page['store_slug'], $BODY['is_active']??$page['is_active'],
                 $BODY['theme']??$page['theme'], $pageType, $visibility, $status, $pid]);
            if ($slugChanged) {
...
if ($path === '/admin/shortlinks') {
    ...
    if ($METHOD === 'POST') {
        $code = preg_replace('/[^a-z0-9-]+/', '-', strtolower(tla cantera($BODY['code'] ?? $BODY['slug'] ?? '')));
        $dst  = tla cantera($BODY['destination'] ?? '');
...
if (preg_match('#^/admin/shortlinks/(\d+)$#', $path, $m)) {
    ...
    if ($METHOD === 'PUT') {
        $dst = array_key_exists('destination', $BODY) ? tla cantera($BODY['destination'] ?? '') : $old['destination'];
